...
struct xag_mingc_rewrite_params
{
	bool verbose{ false };
...
struct xag_mingc_rewrite_stats
{
...
class xag_mingc_rewrite
{
public:
	xag_mingc_rewrite( std::string const& dbname, xag_mingc_rewrite_params const& ps = {},
										 xag_mingc_rewrite_stats* pst = nullptr )
		: ps_( ps ),
			pst_( pst ),
...
private:
	void build_db( std::string const& dbname )
	{
		...
		while ( std::getline( db_gc, line ) )
		{
			...
			std::vector<xag_network::signal> signals_gc_opt( db_pis_.begin(), db_pis_.begin() + num_vars );
	    	...
	    	{
	    		signal1 = signals_gc_opt[signal_1 / 2 - 1] ^ ( signal_1 % 2 != 0 );
	    	}
	    	if ( signal_2 == 0u )
	    	{
	    	...
	    	{
	    		signal2 = signals_gc_opt[signal_2 / 2 - 1] ^ ( signal_2 % 2 != 0 );
	    	}

	    	if ( signal_1 > signal_2 )
	    	{
	    		signals_gc_opt.emplace_back( db_.create_xor( signal1, signal2 ) );
	    	}
	    	else
	    	{
	    		signals_gc_opt.emplace_back( db_.create_and( signal1, signal2 ) );
	    	}
			}

			const uint32_t signal_po = std::stoul( line );
			const xag_network::signal po = signals_gc_opt[signal_po / 2 - 1] ^ ( signal_po % 2 != 0 );
			db_.create_po( po );
...
private:
	...
	std::shared_ptr<std::unordered_map<std::string, std::tuple<std::string, uint32_t, xag_network::signal>>> pfunc_gc_;
	...
	xag_mingc_rewrite_params const& ps_;
	//xag_mingc_rewrite_stats st_;
	xag_mingc_rewrite_stats* pst_{ nullptr };
};
