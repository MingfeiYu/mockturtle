...
struct optimum_gc_synthesis_stats
{
...
struct optimum_gc_synthesis_impl
{
  using problem_network_t = cnf_view<xag_network, false, Solver>;

public:
  optimum_gc_synthesis_impl( kitty::dynamic_truth_table const& func, std::vector<uint32_t> num_ands,
                             optimum_gc_synthesis_params const& ps, optimum_gc_synthesis_stats& st )
...
private:
  ...
  Ntk extract_network( problem_network_t& pntk )
  {
    ...
    {
      auto result = simulate<kitty::static_truth_table<6u>>( ntk )[0];
...
private:
  ...
  optimum_gc_synthesis_params const& ps_;
  optimum_gc_synthesis_stats& st_;
};

} /* namespace detail */

template<class Ntk = xag_network, bill::solvers Solver = bill::solvers::glucose_41>
std::optional<Ntk> optimum_gc_synthesis( kitty::dynamic_truth_table const& func, std::vector<uint32_t> num_ands, optimum_gc_synthesis_params const& ps = {}, optimum_gc_synthesis_stats* pst = nullptr )
{
  optimum_gc_synthesis_stats st;

  const auto pntk = detail::optimum_gc_synthesis_impl<Ntk, Solver>{ func, num_ands, ps, st }.run();
