...
/* Generate db_gc for the the 5000 popular functions in epfl benchmarks */
/* Only functions with mc no more than 5 are taken into consideration   */
void create_db_x1g_epfl_thread0( std::string const& filename )
{
    ...
    std::ofstream db_gc;
    db_gc.open( "db_gc_x1g_epfl_thread0", std::ios::app );
    db_gc << "0x" << repr_str << " ";
    db_gc << "0x" << tt_str << " ";
    //db_gc << mc << " ";
    db_gc << num_vars << " ";
...
int main()
{
	mockturtle::create_db_x1g_epfl_thread0( "collected_funcs_epfl" );
